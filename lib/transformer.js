...
class Transformer {
     ...
     * @returns 转换后的代码
     */
    transformJsFileToI18n(source, options = {}) {
     ...
     * @returns 转换后的代码
     */
    transformVueFileToI18n(source, options = {}) {
        ...
        if (templateMatch) {
            ...
            transformedTemplate = transformedTemplate.replace(/(\s\w+)=["']([^"']*[\u4e00-\u9fa5][^"']*)["']/g, (match, attr, value) => {
                if (types_1.CHINESE_REGEX.test(value)) {
            ...
            transformedTemplate = transformedTemplate.replace(/(\sv-(text|html|bind:[^=]+))=["']'([^"']*[\u4e00-\u9fa5][^"']*)'["']/g, (match, directive, _, value) => {
                if (types_1.CHINESE_REGEX.test(value)) {
            ...
            transformedTemplate = transformedTemplate.replace(/>([^<{]*[\u4e00-\u9fa5][^<{]*)</g, (match, text) => {
                if (types_1.CHINESE_REGEX.test(text.trim())) {
                    return `>{{ ${functionName}('${text.trim().replace(/'/g, "\\'")}')}}</`;
                }
                return match;
            });
            // 4. 处理包含中文的 {{ }} 表达式内的字符串字面量
            transformedTemplate = transformedTemplate.replace(/{{([^}]*['"]([^'"]*[\u4e00-\u9fa5][^'"]*)['"](.*?))}}/g, (match, expr, chineseText) => {
                if (types_1.CHINESE_REGEX.test(chineseText)) {
                    // 如果表达式中包含中文字符串字面量，替换为 $t 调用
                    const modifiedExpr = expr.replace(/(['"])([^'"]*[\u4e00-\u9fa5][^'"]*)(['"])/g, (_, quote1, text, quote2) => {
            ...
            (0, traverse_1.default)(ast, {
                StringLiteral(path) {
                    ...
                    if (types_1.CHINESE_REGEX.test(node.value)) {
                        ...
                        // 替换为t函数调用
                        const i18nCall = t.callExpression(t.identifier(functionName), [t.stringLiteral(node.value)]);
                ...
                JSXAttribute(path) {
                    ...
                    if (t.isStringLiteral(node.value) &&
                        types_1.CHINESE_REGEX.test(node.value.value)) {
                        // 替换为JSX表达式，包含t函数调用
                        const i18nCall = t.callExpression(t.identifier(functionName), [t.stringLiteral(node.value.value)]);
                ...
                JSXText(path) {
                    ...
                    if (text && types_1.CHINESE_REGEX.test(text)) {
                        // 替换为JSX表达式，包含t函数调用
                        const i18nCall = t.callExpression(t.identifier(functionName), [t.stringLiteral(text)]);
 ...
 * @returns 是否在函数调用中
 */
function isInI18nCall(path, functionName) {
...
// 导出单例
exports.transform = new Transformer();
