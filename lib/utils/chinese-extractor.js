...
const extraction_reporter_1 = require("./extraction-reporter");
class ChineseExtractor {
    constructor(options = {}) {
        this.reporter = new extraction_reporter_1.ExtractionReporter();
    ...
    extractFromVueFile(source) {
        ...
        for (const text of results) {
            ...
            if (this.debug)
                console.log('[extractor][vue][filter]', valid ? 'keep' : 'skip', JSON.stringify(text));
            if (valid)
                filtered.push(text);
        }
        return filtered;
    ...
    isValidChineseText(text) {
        ...
        if (chineseCharCount < 2) {
            if (this.debug)
                console.log('[extractor][rule] <2 chinese chars', JSON.stringify(text));
            return false;
        }
        ...
        if (chineseCharCount / totalLength < 0.1) {
            if (this.debug)
                console.log('[extractor][rule] ratio <0.1', JSON.stringify(text));
            return false;
        }
        return true;
    }
    ...
    extractFromTemplate(template) {
            ...
            if (text && types_1.CHINESE_REGEX.test(text) && !text.includes('\r') && !text.includes('\n')) {
                ...
                if (text.includes('{{')) {
                    ...
                    for (const seg of segments) {
                        if (seg && types_1.CHINESE_REGEX.test(seg)) {
                    ...
                    if (this.debug)
                        console.log('[extractor][template-text]', JSON.stringify(text));
                }
            }
        }
        ...
        while ((match = attrValueRegex.exec(template)) !== null) {
            ...
            if (text && types_1.CHINESE_REGEX.test(text)) {
                chineseTexts.add(text);
                if (this.debug)
                    console.log('[extractor][attr]', JSON.stringify(text));
            }
        }
        // 3. 提取Vue指令中的字符串字面量
        ...
        while ((match = vueDirectiveRegex.exec(template)) !== null) {
            ...
            if (text && types_1.CHINESE_REGEX.test(text)) {
                chineseTexts.add(text);
                if (this.debug)
                    console.log('[extractor][directive]', JSON.stringify(text));
            }
        }
        ...
        while ((match = stringLiteralRegex.exec(template)) !== null) {
            ...
            if (text && types_1.CHINESE_REGEX.test(text) && !text.includes('<') && !text.includes('>')) {
                chineseTexts.add(text);
                if (this.debug)
                    console.log('[extractor][template-string]', JSON.stringify(text));
            }
        }
            ...
            for (const block of interpolationBlocks) {
                ...
                while ((interpolationMatch = stringInInterpolation.exec(block)) !== null) {
                    ...
                    if (text && types_1.CHINESE_REGEX.test(text)) {
                        chineseTexts.add(text);
                        if (this.debug)
                            console.log('[extractor][interpolation-string]', JSON.stringify(text));
                    }
                }
            }
        }
        ...
        while ((wrappedMatch = wrappedCallRegex.exec(template)) !== null) {
            ...
            if (txt) {
                chineseTexts.add(txt);
                if (this.debug)
                    console.log('[extractor][wrapped-call]', JSON.stringify(txt));
            }
        }
        ...
        try {
            ...
            (0, traverse_1.default)(ast, {
                ...
                StringLiteral: (path) => {
                    ...
                    if (types_1.CHINESE_REGEX.test(node.value)) {
                        ...
                        if (this.debug)
                            console.log('[extractor][script-string]', JSON.stringify(node.value));
                    }
                },
                // 处理模板字面量
                        ...
                        if (types_1.CHINESE_REGEX.test(quasi.value.raw)) {
                            ...
                            for (const segment of chineseSegments) {
                                if (segment.trim()) {
                                    ...
                                    if (this.debug)
                                        console.log('[extractor][script-template-seg]', JSON.stringify(segment));
                                }
                            }
                        }
                    }
                        ...
                        if (trimmed) {
                            ...
                            for (const segment of chineseSegments) {
                                if (segment.trim()) {
                                    ...
                                    if (this.debug)
                                        console.log('[extractor][jsx-text]', JSON.stringify(segment));
                                }
                            }
                        }
                    }
                ...
                JSXAttribute: (path) => {
                    ...
                    if (t.isStringLiteral(node.value) && types_1.CHINESE_REGEX.test(node.value.value)) {
                        ...
                        if (this.debug)
                            console.log('[extractor][jsx-attr]', JSON.stringify(node.value.value));
                    }
                }
            });
            ...
            while ((jsMatch = wrappedCallRegexJs.exec(source)) !== null) {
                ...
                if (inner) {
                    ...
                    if (this.debug)
                        console.log('[extractor][script-wrapped-call]', JSON.stringify(inner));
                }
            }
        }
        catch (error) {
