...
exports.RenderDetector = void 0;
class RenderDetector {
    /**
     * 检查AST是否包含render函数
     */
    checkForRenderFunction(ast) {
        if (!ast || !ast.body)
            return false;
     ...
     */
    checkForRenderInEmittedCode(source) {
     ...
     * 检查资源是否应该被转换
     */
    shouldTransformFile(resource, excludePatterns = []) {
        if (!resource)
            return false;
...
exports.RenderDetector = RenderDetector;
