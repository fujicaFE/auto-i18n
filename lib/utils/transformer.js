            ...
            (0, traverse_1.default)(ast, {
                ...
                StringLiteral(path) {
                    ...
                    if (translations[node.value]) {
                        ...
                        // 替换为$t函数调用
                        const i18nCall = t.callExpression(t.identifier(self.options.functionName || '$t'), [t.stringLiteral(node.value)]);
                ...
                JSXAttribute(path) {
                    ...
                    if (t.isStringLiteral(node.value) && translations[node.value.value]) {
                        // 替换为JSX表达式，内部是$t函数调用
                        const i18nCall = t.callExpression(t.identifier(self.options.functionName || '$t'), [t.stringLiteral(node.value.value)]);
                ...
                JSXText(path) {
                    ...
                    if (text && translations[text]) {
                        // 创建i18n函数调用
                        const i18nCall = t.callExpression(t.identifier(self.options.functionName || '$t'), [t.stringLiteral(text)]);
                        // 用JSXExpressionContainer包装
                        const jsxExpr = t.jSXExpressionContainer(i18nCall);
                        // 替换JSX文本
